...
begin
  Enabled := True;
  ShowCursor(False);
  RenderTimer.Enabled := False;// долждно быть true
end;
...
begin
  wglMakeCurrent(0, 0);
  @InitLibrary := GetProcAddress(Handles[1], 'InitLibrary');
...
begin
  glViewport(0, 0, ClientWidth, ClientHeight);  //????Возможно из за этого зависает
